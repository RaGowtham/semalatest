...
static int adl_bmc_vm_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < vm_data->cnt; i++) 
        {
		vm_data->adl_bmc_vm_rdev[i] = regulator_register(&vm_data->adl_bmc_vm_desc[i], &config);
...
ret_failed:
	for(j = 0; j < i; j++) {
		regulator_unregister(vm_data->adl_bmc_vm_rdev[j]);
...
static int adl_bmc_vm_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < vm_data->cnt; i++) 
	{
		regulator_unregister(vm_data->adl_bmc_vm_rdev[i]);
	}
...
void regulator_dummy_init(void)
{
	int ret;
